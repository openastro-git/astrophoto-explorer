...
from .thumbnail_generator import generate_thumbnail, THUMBS_DIR
...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["X-Image-Shape", "X-Image-Stretch", "X-Image-Size", "X-Image-FileType", "X-Image-Object", "X-Image-Astrometry"]
)
...
async def websocket_scan(websocket: WebSocket):
    ...
    try:
        ...
        print(f"[WS] Scan complete, sending result with {result.get('count', 0)} files, {len(result.get('groups', []))} groups")
...
async def scan_folder(path: str = ".", force: bool = False):
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            pass
    return await scan_fits_folder(
        path, use_cache=not force,
        scan_pattern=scan_pattern, ignore_filters=ignore_filters
    )
...
async def test_scan_pattern(data: dict):
    ...
    for file_path_found, file_type in all_files:
        try:
            ...
            results.append({
                "relative_path": str(file_path_found),
                "error": str(e)
            })
...
def preview_image(file_path: str, size: str = "medium", stretch: str = "mtf"):
    ...
    if 'astrometry' in data:
        ...
        print(f"[PREVIEW API] No astrometry data in response")
