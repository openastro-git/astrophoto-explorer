...
def update_file_metadata_in_cache(file_path: str):
    ...
    try:
        ...
        print(f"[CACHE UPDATE] Fresh metadata: object={fresh_meta.get('object')}, astrometry={fresh_meta.get('astrometry')}")
                ...
                for session in group['sessions']:
                    for file_meta in session.get('files', []):
                        if file_meta['path'] == file_path:
                            ...
                            updated = True
                            updated_group = group
                            print(f"[CACHE UPDATE] Updated file in session")
                            break
...
async def scan_fits_folder(folder_path, use_cache=True, progress_callback=None,
                           scan_pattern=None, ignore_filters=None):
    ...
    if path_pattern and not path_pattern.is_empty:
        ...
        path_pattern = None
        print(f"[SCAN] No scan pattern configured, using auto-detect")
    ...
    print(f"[PERF] Metadata extraction completed in {meta_elapsed:.3f}s ({meta_elapsed/len(all_files)*1000:.1f}ms per file)")
    ...
    for group in grouped_list:
        ...
        for file_meta in group['files']:
            if 'astrometry' in file_meta and file_meta['astrometry']:
                ...
                if 'ra' in astro_data and 'dec' in astro_data and astro_data['ra'] is not None and astro_data['dec'] is not None:
        ...
        if not astrometry:
            ...
            if obj_info and 'ra' in obj_info and 'dec' in obj_info:
                ...
                if is_ngc896:
                    print(f"[NGC0896 DEBUG] [OK] Got coordinates from catalog: ra={astrometry['ra']}, dec={astrometry['dec']}")
            elif is_ngc896:
                print(f"[NGC0896 DEBUG] [FAIL] Catalog lookup returned no coordinates")
